...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        corners_found = state[2]
        
        if False in corners_found:
            return False
        else:
            return True
    ...
    def getSuccessors(self, state):
        ...
        x,y,z = state
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            corners_found = list(z)
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            nextState = (nextx, nexty)
            
            #Change corners_found if new state has hit a corner
            try: 
            ...
            #Add new state to successors if move is legal
            if not self.walls[nextx][nexty]:
...
def cornersHeuristic(state, problem):
    ...
    x,y,found_corner = state
     
    max_distance = 0
    for i, corner in enumerate(corners):
        if found_corner[i] == False:
